...
\textbf{Causal identification.} SCM weights concentrate on a single donor county (weight $>0.85$), reducing the method to an elaborated case comparison. ITS estimates lack statistical significance at conventional thresholds, and with $N=27$, statistical power is limited to detecting only large effects ($\geq$0.6 SD). The combined evidence is \emph{mechanism-illustrative} rather than \emph{causal-confirmatory}.
